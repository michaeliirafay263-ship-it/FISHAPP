...
export function StockPriceManager() {
  const { fishCatalog, updateDailyFish, addNewFish } = useAdminData();
  ...
  const handleAddNewSubmit = (e) => {
    ...
    addNewFish({
      ...
      weightStep: 0.5,
      source: newSource,
      description: newDescription || `${newName} fresh from morning landings.`,
      image: newImage,
      badge: 'New Arrival',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h2 style={{ fontSize: '1.3rem', fontWeight: '800', color: 'var(--color-navy)' }}>
            Daily Stock & Price Control Center
          </h2>
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-subtle)' }}>
            Adjust daily Kivukoni auction prices and available stock kg reflected live across the platform.
          </p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
          <thead>
            <tr style={{ backgroundColor: '#0f172a', color: '#ffffff' }}>
              ...
              <th style={{ padding: '12px 16px' }}>Source / Landing</th>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {isEditing ? (
                      ...
                      <span className={fish.inStock ? 'badge badge-success' : 'badge badge-danger'}>
                        {fish.inStock ? 'In Stock' : 'Sold Out Today'}
                      </span>
                    )}
                  </td>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '540px' }}>
            <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ fontSize: '1.15rem', fontWeight: '800', color: 'var(--color-navy)' }}>
                Add New Fish to Daily Inventory
              </h3>
